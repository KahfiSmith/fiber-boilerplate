...
func (r *authSessionRepository) HasSession(ctx context.Context, userID uint, sessionID string) (bool, error) {
	var count int64
	if err := r.db.WithContext(ctx).
		Model(&models.AuthSession{}).
		Where("user_id = ? AND session_id = ? AND expires_at > ?", userID, strings.TrimSpace(sessionID), time.Now().UTC()).
		Count(&count).
		Error; err != nil {
